...
superhack::App::App(Process *process)
{
...
int superhack::App::exec()
{
    if (!m_controller->isReady()) {
        LOG_EMERG("no valid configuration found, try https://superhack.com/wizard");
...
void superhack::App::onConsoleCommand(char command)
{
    if (command == 3) {
...
void superhack::App::onSignal(int signum)
{
    switch (signum)
    {
    case SIGHUP:
