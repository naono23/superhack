...
{
public:
    superhack_DISABLE_COPY_MOVE_DEFAULT(RxNUMAStoragePrivate)
...
superhack::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


superhack::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool superhack::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


superhack::HugePagesInfo superhack::RxNUMAStorage::hugePages() const
{
...
superhack::RxDataset *superhack::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void superhack::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
