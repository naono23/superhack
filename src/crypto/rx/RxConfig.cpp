...
bool superhack::RxConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
rapidjson::Value superhack::RxConfig::toJSON(rapidjson::Document &doc) const
{
...
#ifdef superhack_FEATURE_HWLOC
std::vector<uint32_t> superhack::RxConfig::nodeset() const
{
    if (!m_nodeset.empty()) {
        return m_nodeset;
    }
...
const char *superhack::RxConfig::modeName() const
{
    return modeNames[m_mode];
}


uint32_t superhack::RxConfig::threads(uint32_t limit) const
{
    if (m_threads > 0) {
        return m_threads;
    }
...
#ifdef superhack_FEATURE_MSR
const char *superhack::RxConfig::msrPresetName() const
{
    return modNames[msrMod()];
}


const superhack::MsrItems &superhack::RxConfig::msrPreset() const
{
...
uint32_t superhack::RxConfig::msrMod() const
{
    if (!wrmsr()) {
...
void superhack::RxConfig::readMSR(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
superhack::RxConfig::Mode superhack::RxConfig::readMode(const rapidjson::Value &value) const
{
    if (value.IsUint()) {
