...
superhack::RxDataset::RxDataset(bool hugePages, bool oneGbPages, bool cache, RxConfig::Mode mode, uint32_t node) :
    m_mode(mode),
    m_node(node)
{
...
superhack::RxDataset::RxDataset(RxCache *cache) :
    m_node(0),
    m_cache(cache)
{
}
...
bool superhack::RxDataset::init(const Buffer &seed, uint32_t numThreads, int priority)
{
...
bool superhack::RxDataset::isHugePages() const
{
    return m_memory && m_memory->isHugePages();
}


bool superhack::RxDataset::isOneGbPages() const
{
    return m_memory && m_memory->isOneGbPages();
}


superhack::HugePagesInfo superhack::RxDataset::hugePages(bool cache) const
{
...
size_t superhack::RxDataset::size(bool cache) const
{
    size_t size = 0;
...
void *superhack::RxDataset::raw() const
{
    return m_dataset ? randomx_get_dataset_memory(m_dataset) : nullptr;
}


void superhack::RxDataset::setRaw(const void *raw)
{
    if (!m_dataset) {
        return;
    }
...
void superhack::RxDataset::allocate(bool hugePages, bool oneGbPages)
{
...
uint8_t* superhack::RxDataset::tryAllocateScrathpad()
{
