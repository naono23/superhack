...
superhack::RxCache::RxCache(bool hugePages, uint32_t nodeId)
{
...
superhack::RxCache::RxCache(uint8_t *memory)
{
    create(memory);
}


superhack::RxCache::~RxCache()
{
...
bool superhack::RxCache::init(const Buffer &seed)
{
    if (m_seed == seed) {
        return false;
    }
...
superhack::HugePagesInfo superhack::RxCache::hugePages() const
{
    return m_memory ? m_memory->hugePages() : HugePagesInfo();
}


void superhack::RxCache::create(uint8_t *memory)
{
    if (!memory) {
        return;
    }
