...
#if defined(superhack_OS_LINUX)
#   if (defined(MAP_HUGE_1GB) || defined(MAP_HUGE_SHIFT))
#       define superhack_HAS_1GB_PAGES
#   endif
#   include "crypto/common/LinuxMemory.h"
#endif


bool superhack::VirtualMemory::isHugepagesAvailable()
{
    return true;
}


bool superhack::VirtualMemory::isOneGbPagesAvailable()
{
#   ifdef superhack_HAS_1GB_PAGES
...
void *superhack::VirtualMemory::allocateExecutableMemory(size_t size)
{
#   if defined(__APPLE__)
...
void *superhack::VirtualMemory::allocateLargePagesMemory(size_t size)
{
#   if defined(__APPLE__)
...
void *superhack::VirtualMemory::allocateOneGbPagesMemory(size_t size)
{
#   ifdef superhack_HAS_1GB_PAGES
...
void superhack::VirtualMemory::flushInstructionCache(void *p, size_t size)
{
...
void superhack::VirtualMemory::freeLargePagesMemory(void *p, size_t size)
{
    munmap(p, size);
}


void superhack::VirtualMemory::protectExecutableMemory(void *p, size_t size)
{
    mprotect(p, size, PROT_READ | PROT_EXEC);
}


void superhack::VirtualMemory::unprotectExecutableMemory(void *p, size_t size)
{
    mprotect(p, size, PROT_WRITE | PROT_EXEC);
}


void superhack::VirtualMemory::osInit(bool)
{
}


bool superhack::VirtualMemory::allocateLargePagesMemory()
{
#   if defined(superhack_OS_LINUX)
...
bool superhack::VirtualMemory::allocateOneGbPagesMemory()
{
#   if defined(superhack_HAS_1GB_PAGES)
...
void superhack::VirtualMemory::freeLargePagesMemory()
{
