...
superhack::VirtualMemory::VirtualMemory(size_t size, bool hugePages, bool oneGbPages, bool usePool, uint32_t node, size_t alignSize) :
    m_size(align(size)),
    m_capacity(m_size),
    m_node(node)
{
...
superhack::VirtualMemory::~VirtualMemory()
{
    if (!m_scratchpad) {
        return;
    }
...
superhack::HugePagesInfo superhack::VirtualMemory::hugePages() const
{
    return { this };
}


#ifndef superhack_FEATURE_HWLOC
uint32_t superhack::VirtualMemory::bindToNUMANode(int64_t)
{
    return 0;
}
#endif
...
void superhack::VirtualMemory::destroy()
{
    delete pool;
}


void superhack::VirtualMemory::init(size_t poolSize, bool hugePages)
{
    if (!pool) {
        osInit(hugePages);
    }
