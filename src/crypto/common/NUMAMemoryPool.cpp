...
superhack::NUMAMemoryPool::NUMAMemoryPool(size_t size, bool hugePages) :
...
superhack::NUMAMemoryPool::~NUMAMemoryPool()
{
    for (auto kv : m_map) {
        delete kv.second;
    }
...
bool superhack::NUMAMemoryPool::isHugePages(uint32_t node) const
{
    if (!m_size) {
        return false;
    }
...
uint8_t *superhack::NUMAMemoryPool::get(size_t size, uint32_t node)
{
    if (!m_size) {
        return nullptr;
    }
...
void superhack::NUMAMemoryPool::release(uint32_t node)
{
...
superhack::IMemoryPool *superhack::NUMAMemoryPool::get(uint32_t node) const
{
    return m_map.count(node) ? m_map.at(node) : nullptr;
}


superhack::IMemoryPool *superhack::NUMAMemoryPool::getOrCreate(uint32_t node) const
{
    auto pool = get(node);
    if (!pool) {
