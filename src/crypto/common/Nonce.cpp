...
uint32_t superhack::Nonce::next(uint8_t index, uint32_t nonce, uint32_t reserveCount, bool nicehash, bool *ok)
{
    uint32_t next;
...
void superhack::Nonce::reset(uint8_t index)
{
    std::lock_guard<std::mutex> lock(mutex);
