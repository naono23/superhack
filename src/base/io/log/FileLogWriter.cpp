...
bool superhack::FileLogWriter::open(const char *fileName)
{
...
bool superhack::FileLogWriter::write(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
bool superhack::FileLogWriter::writeLine(const char *data, size_t size)
{
    uv_buf_t buf[2] = {
