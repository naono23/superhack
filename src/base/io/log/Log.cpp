...
{
public:
    superhack_DISABLE_COPY_MOVE(LogPrivate)
...
void superhack::Log::add(ILogBackend *backend)
{
    if (d) {
        d->add(backend);
    }
...
void superhack::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void superhack::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
