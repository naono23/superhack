...
superhack::JsonChain::JsonChain() = default;


bool superhack::JsonChain::add(rapidjson::Document &&doc)
{
...
bool superhack::JsonChain::addFile(const char *fileName)
{
...
bool superhack::JsonChain::addRaw(const char *json)
{
    using namespace rapidjson;
    Document doc;
...
void superhack::JsonChain::dump(const char *fileName)
{
...
bool superhack::JsonChain::getBool(const char *key, bool defaultValue) const
{
...
const char *superhack::JsonChain::getString(const char *key, const char *defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &superhack::JsonChain::getArray(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &superhack::JsonChain::getObject(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &superhack::JsonChain::getValue(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
        if (i != it->MemberEnd()) {
...
int superhack::JsonChain::getInt(const char *key, int defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
int64_t superhack::JsonChain::getInt64(const char *key, int64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
uint64_t superhack::JsonChain::getUint64(const char *key, uint64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
unsigned superhack::JsonChain::getUint(const char *key, unsigned defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
