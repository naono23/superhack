...
static const int signums[superhack::Signals::kSignalsCount] = { SIGHUP, SIGINT, SIGTERM };


superhack::Signals::Signals(ISignalListener *listener)
    : m_listener(listener)
{
...
superhack::Signals::~Signals()
{
    stop();
}


void superhack::Signals::stop()
{
    if (!m_signals[0]) {
        return;
    }
...
void superhack::Signals::onSignal(uv_signal_t *handle, int signum)
{
