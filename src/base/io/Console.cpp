...
superhack::Console::Console(IConsoleListener *listener)
    : m_listener(listener)
{
    if (!isSupported()) {
        return;
...
superhack::Console::~Console()
{
    stop();
}


void superhack::Console::stop()
{
    if (!m_tty) {
        return;
    }
...
bool superhack::Console::isSupported() const
{
...
void superhack::Console::onAllocBuffer(uv_handle_t *handle, size_t, uv_buf_t *buf)
{
...
void superhack::Console::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    if (nread < 0) {
