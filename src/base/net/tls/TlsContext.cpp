...
superhack::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


superhack::TlsContext *superhack::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool superhack::TlsContext::load(const TlsConfig &config)
{
...
bool superhack::TlsContext::setCiphers(const char *ciphers)
{
...
bool superhack::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool superhack::TlsContext::setDH(const char *dhparam)
{
    DH *dh = nullptr;
...
void superhack::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
