...
superhack::ServerTls::ServerTls(SSL_CTX *ctx) :
    m_ctx(ctx)
{
}


superhack::ServerTls::~ServerTls()
{
    if (m_ssl) {
        SSL_free(m_ssl);
    }
...
bool superhack::ServerTls::isTLS(const char *data, size_t size)
{
...
bool superhack::ServerTls::send(const char *data, size_t size)
{
...
void superhack::ServerTls::read(const char *data, size_t size)
{
    if (!m_ssl) {
        m_ssl = SSL_new(m_ctx);
