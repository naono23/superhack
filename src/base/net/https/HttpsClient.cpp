...
superhack::HttpsClient::HttpsClient(FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
superhack::HttpsClient::~HttpsClient()
{
    if (m_ctx) {
        SSL_CTX_free(m_ctx);
    }
...
const char *superhack::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *superhack::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void superhack::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void superhack::HttpsClient::read(const char *data, size_t size)
{
...
void superhack::HttpsClient::write(std::string &&data, bool close)
{
...
bool superhack::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool superhack::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void superhack::HttpsClient::flush(bool close)
{
