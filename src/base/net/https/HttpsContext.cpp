...
superhack::HttpsContext::HttpsContext(TlsContext *tls, const std::weak_ptr<IHttpListener> &listener) :
...
superhack::HttpsContext::~HttpsContext() = default;


void superhack::HttpsContext::append(char *data, size_t size)
{
    if (m_mode == TLS_AUTO) {
...
bool superhack::HttpsContext::write(BIO *bio)
{
...
void superhack::HttpsContext::parse(char *data, size_t size)
{
...
void superhack::HttpsContext::shutdown()
{
    close();
}


void superhack::HttpsContext::write(std::string &&data, bool close)
{
    m_close = close;
