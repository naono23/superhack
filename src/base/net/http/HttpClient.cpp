...
superhack::HttpClient::HttpClient(FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
superhack::HttpClient::~HttpClient()
{
    delete m_dns;
}


bool superhack::HttpClient::connect()
{
    return m_dns->resolve(m_req.host);
}


void superhack::HttpClient::onResolved(const Dns &dns, int status)
{
    this->status = status;
...
void superhack::HttpClient::handshake()
{
...
void superhack::HttpClient::read(const char *data, size_t size)
{
...
void superhack::HttpClient::onConnect(uv_connect_t *req, int status)
{
