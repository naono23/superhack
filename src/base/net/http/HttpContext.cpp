...
{
public:
    superhack_DISABLE_COPY_MOVE_DEFAULT(HttpWriteBaton)
...
superhack::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
...
superhack::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}
...
void superhack::HttpContext::write(std::string &&data, bool close)
{
...
bool superhack::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}


size_t superhack::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return size;
    }
...
std::string superhack::HttpContext::ip() const
{
    char ip[46]           = {};
...
uint64_t superhack::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}


void superhack::HttpContext::close(int status)
{
...
superhack::HttpContext *superhack::HttpContext::get(uint64_t id)
{
...
void superhack::HttpContext::closeAll()
{
    for (auto &kv : storage) {
...
int superhack::HttpContext::onHeaderField(http_parser *parser, const char *at, size_t length)
{
...
int superhack::HttpContext::onHeaderValue(http_parser *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void superhack::HttpContext::attach(http_parser_settings *settings)
{
...
void superhack::HttpContext::setHeader()
{
