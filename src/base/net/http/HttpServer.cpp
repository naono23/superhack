...
superhack::HttpServer::HttpServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


superhack::HttpServer::~HttpServer()
{
    HttpContext::closeAll();
}


void superhack::HttpServer::onConnection(uv_stream_t *stream, uint16_t)
{
