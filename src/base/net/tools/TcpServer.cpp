...
static const superhack::String kLocalHost("127.0.0.1");


superhack::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
superhack::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int superhack::TcpServer::bind()
{
    if (!m_version) {
...
void superhack::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void superhack::TcpServer::onConnection(uv_stream_t *stream, int status)
{
