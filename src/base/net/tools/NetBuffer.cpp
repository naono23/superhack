...
char *superhack::NetBuffer::allocate()
{
    return getPool()->allocate();
}


void superhack::NetBuffer::destroy()
{
    if (!pool) {
        return;
    }
...
void superhack::NetBuffer::onAlloc(uv_handle_t *, size_t, uv_buf_t *buf)
{
...
void superhack::NetBuffer::release(const char *buf)
{
    getPool()->deallocate(buf);
}


void superhack::NetBuffer::release(const uv_buf_t *buf)
{
