...
superhack::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener) :
    m_listener(listener)
{
...
superhack::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


void superhack::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void superhack::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void superhack::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool superhack::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void superhack::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void superhack::SelfSelectClient::retry()
{
    setState(RetryState);
}


void superhack::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void superhack::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void superhack::SelfSelectClient::onHttpData(const HttpData &data)
{
