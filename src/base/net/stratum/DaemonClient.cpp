...
superhack::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
superhack::DaemonClient::~DaemonClient()
{
    delete m_timer;
}


bool superhack::DaemonClient::disconnect()
{
...
bool superhack::DaemonClient::isTLS() const
{
...
int64_t superhack::DaemonClient::submit(const JobResult &result)
{
...
void superhack::DaemonClient::connect()
{
...
void superhack::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void superhack::DaemonClient::onHttpData(const HttpData &data)
{
...
void superhack::DaemonClient::onTimer(const Timer *)
{
...
bool superhack::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash;
}


bool superhack::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool superhack::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t superhack::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t superhack::DaemonClient::rpcSend(const rapidjson::Document &doc)
{
...
void superhack::DaemonClient::retry()
{
    m_failures++;
...
void superhack::DaemonClient::send(const char *path)
{
...
void superhack::DaemonClient::setState(SocketState state)
{
