...
superhack::Client::Client(int id, const char *agent, IClientListener *listener) :
...
bool superhack::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool superhack::Client::isTLS() const
{
#   ifdef superhack_FEATURE_TLS
...
const char *superhack::Client::tlsFingerprint() const
{
#   ifdef superhack_FEATURE_TLS
...
const char *superhack::Client::tlsVersion() const
{
#   ifdef superhack_FEATURE_TLS
    if (isTLS()) {
...
int64_t superhack::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t superhack::Client::send(const rapidjson::Value &obj)
{
...
int64_t superhack::Client::submit(const JobResult &result)
{
#   ifndef superhack_PROXY_PROJECT
...
void superhack::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void superhack::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void superhack::Client::tick(uint64_t now)
{
...
void superhack::Client::onResolved(const Dns &dns, int status)
{
...
bool superhack::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
bool superhack::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   endif
    ...
    if (m_jobs == 0) { // https://github.com/superhack/superhack/issues/459
        return false;
    }
...
bool superhack::Client::send(BIO *bio)
{
#   ifdef superhack_FEATURE_TLS
    uv_buf_t buf;
...
bool superhack::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool superhack::Client::write(const uv_buf_t &buf)
{
...
int superhack::Client::resolve(const String &host)
{
...
int64_t superhack::Client::send(size_t size)
{
...
void superhack::Client::connect(sockaddr *addr)
{
...
void superhack::Client::handshake()
{
    if (m_socks5) {
...
bool superhack::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void superhack::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void superhack::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void superhack::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void superhack::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void superhack::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void superhack::Client::reconnect()
{
    if (!m_listener) {
        m_storage.remove(m_key);
...
void superhack::Client::setState(SocketState state)
{
...
void superhack::Client::startTimeout()
{
    m_expire = 0;
...
void superhack::Client::onClose(uv_handle_t *handle)
{
...
void superhack::Client::onConnect(uv_connect_t *req, int status)
{
...
void superhack::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
