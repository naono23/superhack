...
bool superhack::Pools::isEqual(const Pools &other) const
{
...
superhack::IStrategy *superhack::Pools::createStrategy(IStrategyListener *listener) const
{
    if (active() == 1) {
...
rapidjson::Value superhack::Pools::toJSON(rapidjson::Document &doc) const
{
...
size_t superhack::Pools::active() const
{
    size_t count = 0;
    for (const Pool &pool : m_data) {
        if (pool.isEnabled()) {
...
void superhack::Pools::load(const IJsonReader &reader)
{
    m_data.clear();
...
void superhack::Pools::print() const
{
    size_t i = 1;
    for (const Pool &pool : m_data) {
...
void superhack::Pools::setDonateLevel(int level)
{
...
void superhack::Pools::setProxyDonate(int value)
{
    switch (value) {
...
void superhack::Pools::setRetries(int retries)
{
...
void superhack::Pools::setRetryPause(int retryPause)
{
