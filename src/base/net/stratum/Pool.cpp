...
superhack::Pool::Pool(const char *url) :
...
superhack::Pool::Pool(const char *host, uint16_t port, const char *user, const char *password, int keepAlive, bool nicehash, bool tls, Mode mode) :
...
superhack::Pool::Pool(const rapidjson::Value &object) :
    m_flags(1 << FLAG_ENABLED),
    m_pollInterval(kDefaultPollInterval),
...
bool superhack::Pool::isEnabled() const
{
#   ifndef superhack_FEATURE_TLS
    if (isTLS()) {
        return false;
    }
#   endif

#   ifndef superhack_FEATURE_HTTP
...
bool superhack::Pool::isEqual(const Pool &other) const
{
...
superhack::IClient *superhack::Pool::createClient(int id, IClientListener *listener) const
{
...
rapidjson::Value superhack::Pool::toJSON(rapidjson::Document &doc) const
{
...
std::string superhack::Pool::printableName() const
{
...
#ifdef APP_DEBUG
void superhack::Pool::print() const
{
