...
superhack::EthStratumClient::EthStratumClient(int id, const char *agent, IClientListener *listener) :
    Client(id, agent, listener)
{
}


int64_t superhack::EthStratumClient::submit(const JobResult& result)
{
...
void superhack::EthStratumClient::login()
{
    m_results.clear();
...
void superhack::EthStratumClient::onClose()
{
    m_authorized = false;
    Client::onClose();
}
...
bool superhack::EthStratumClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void superhack::EthStratumClient::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void superhack::EthStratumClient::setExtraNonce(const rapidjson::Value &nonce)
{
    if (!nonce.IsString()) {
...
const char *superhack::EthStratumClient::errorMessage(const rapidjson::Value &error) const
{
...
void superhack::EthStratumClient::authorize()
{
    using namespace rapidjson;
...
void superhack::EthStratumClient::onAuthorizeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    try {
        if (!success) {
...
void superhack::EthStratumClient::onSubscribeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    if (!success) {
        return;
    }
...
void superhack::EthStratumClient::subscribe()
{
    using namespace rapidjson;
