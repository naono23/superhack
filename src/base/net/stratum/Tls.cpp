...
superhack::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool superhack::Client::Tls::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
bool superhack::Client::Tls::send(const char *data, size_t size)
{
...
const char *superhack::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *superhack::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void superhack::Client::Tls::read(const char *data, size_t size)
{
...
bool superhack::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool superhack::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool superhack::Client::Tls::verifyFingerprint(X509 *cert)
{
