...
superhack::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef superhack_FEATURE_API
rapidjson::Value superhack::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value superhack::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void superhack::NetworkState::printConnection() const
{
    if (!m_active) {
...
void superhack::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *superhack::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string superhack::NetworkState::humanDiff(uint64_t diff)
{
...
void superhack::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void superhack::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void superhack::NetworkState::onPause(IStrategy *strategy)
{
...
void superhack::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t superhack::NetworkState::latency() const
{
...
uint64_t superhack::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t superhack::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void superhack::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
...
void superhack::NetworkState::stop()
{
    m_active      = false;
    m_diff        = 0;
    m_ip          = nullptr;
