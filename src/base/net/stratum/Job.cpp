...
superhack::Job::Job(bool nicehash, const Algorithm &algorithm, const String &clientId) :
...
bool superhack::Job::setSeedHash(const char *hash)
{
...
bool superhack::Job::setTarget(const char *target)
{
    if (!target) {
        return false;
    }
...
void superhack::Job::setDiff(uint64_t diff)
{
    m_diff   = diff;
...
void superhack::Job::copy(const Job &other)
{
...
void superhack::Job::move(Job &&other)
{
    m_algorithm  = other.m_algorithm;
    m_nicehash   = other.m_nicehash;
    m_size       = other.m_size;
