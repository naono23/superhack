...
superhack::Dns::Dns(IDnsListener *listener) :
    m_listener(listener)
{
...
bool superhack::Dns::resolve(const String &host)
{
    if (m_host != host) {
        m_host = host;
...
const char *superhack::Dns::error() const
{
    return uv_strerror(m_status);
}


const superhack::DnsRecord &superhack::Dns::get(DnsRecord::Type prefered) const
{
    if (count() == 0) {
...
size_t superhack::Dns::count(DnsRecord::Type type) const
{
    if (type == DnsRecord::A) {
        return m_ipv4.size();
    }
...
void superhack::Dns::onResolved(int status, addrinfo *res)
{
    m_status = status;
...
void superhack::Dns::onResolved(uv_getaddrinfo_t *req, int status, addrinfo *res)
{
