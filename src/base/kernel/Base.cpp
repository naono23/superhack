...
#ifdef superhack_FEATURE_EMBEDDED_CONFIG
...
{
public:
    superhack_DISABLE_COPY_MOVE_DEFAULT(BasePrivate)
...
superhack::Base::Base(Process *process)
...
bool superhack::Base::isReady() const
{
    return d_ptr->config != nullptr;
}


int superhack::Base::init()
{
#   ifdef superhack_FEATURE_API
...
superhack::Api *superhack::Base::api() const
{
...
bool superhack::Base::isBackground() const
{
    return d_ptr->config && d_ptr->config->isBackground();
}


bool superhack::Base::reload(const rapidjson::Value &json)
{
...
superhack::Config *superhack::Base::config() const
{
...
void superhack::Base::addListener(IBaseListener *listener)
{
    d_ptr->listeners.push_back(listener);
}


void superhack::Base::onFileChanged(const String &fileName)
{
...
#ifdef superhack_FEATURE_API
void superhack::Base::onRequest(IApiRequest &request)
{
