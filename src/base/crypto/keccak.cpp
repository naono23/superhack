...
void superhack::keccakf(uint64_t st[25], int rounds)
{
    int i, j, round;
    uint64_t t, bc[5];
...
void superhack::keccak(const uint8_t *in, int inlen, uint8_t *md, int mdlen)
{
    state_t st;
    uint8_t temp[144];
    int i, rsiz, rsizw;
    ...
    for ( ; inlen >= rsiz; inlen -= rsiz, in += rsiz) {
        ...
        superhack::keccakf(st, KECCAK_ROUNDS);
    }
