...
superhack::Algorithm::Algorithm(const rapidjson::Value &value) :
    m_id(parse(value.GetString()))
{
}


rapidjson::Value superhack::Algorithm::toJSON() const
{
...
rapidjson::Value superhack::Algorithm::toJSON(rapidjson::Document &) const
{
    return toJSON();
}


size_t superhack::Algorithm::l2() const
{
#   ifdef superhack_ALGO_RANDOMX
    switch (m_id) {
    case RX_0:
...
size_t superhack::Algorithm::l3() const
{
...
uint32_t superhack::Algorithm::maxIntensity() const
{
#   ifdef superhack_ALGO_RANDOMX
    if (family() == RANDOM_X) {
        return 1;
...
superhack::Algorithm::Family superhack::Algorithm::family(Id id)
{
    switch (id) {
    case CN_0:
    case CN_1:
...
superhack::Algorithm::Id superhack::Algorithm::parse(const char *name)
{
...
const char *superhack::Algorithm::name(bool shortName) const
{
    for (const AlgoName &item : algorithm_names) {
        if (item.id == m_id) {
