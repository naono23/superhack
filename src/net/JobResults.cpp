...
#if defined(superhack_FEATURE_OPENCL) || defined(superhack_FEATURE_CUDA)
...
{
public:
    superhack_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void superhack::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void superhack::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void superhack::JobResults::stop()
{
    assert(handler != nullptr);
...
void superhack::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void superhack::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(superhack_FEATURE_OPENCL) || defined(superhack_FEATURE_CUDA)
void superhack::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
