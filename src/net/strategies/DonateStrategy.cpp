...
superhack::DonateStrategy::DonateStrategy(Controller *controller, IStrategyListener *listener) :
...
superhack::DonateStrategy::~DonateStrategy()
{
    delete m_timer;
    delete m_strategy;
...
int64_t superhack::DonateStrategy::submit(const JobResult &result)
{
    return m_proxy ? m_proxy->submit(result) : m_strategy->submit(result);
}


void superhack::DonateStrategy::connect()
{
    m_proxy = createProxy();
    if (m_proxy) {
        m_proxy->connect();
...
void superhack::DonateStrategy::setAlgo(const superhack::Algorithm &algo)
{
    m_algorithm = algo;
...
void superhack::DonateStrategy::setProxy(const ProxyUrl &proxy)
{
    m_strategy->setProxy(proxy);
}


void superhack::DonateStrategy::stop()
{
    m_timer->stop();
    m_strategy->stop();
}
...
void superhack::DonateStrategy::tick(uint64_t now)
{
    m_now = now;
...
void superhack::DonateStrategy::onActive(IStrategy *, IClient *client)
{
    if (isActive()) {
        return;
    }
...
void superhack::DonateStrategy::onPause(IStrategy *)
{
}


void superhack::DonateStrategy::onClose(IClient *, int failures)
{
...
void superhack::DonateStrategy::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
void superhack::DonateStrategy::onLogin(IStrategy *, IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
    setAlgorithms(doc, params);
}


void superhack::DonateStrategy::onLoginSuccess(IClient *client)
{
    if (isActive()) {
        return;
    }
...
void superhack::DonateStrategy::onVerifyAlgorithm(const IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void superhack::DonateStrategy::onVerifyAlgorithm(IStrategy *, const  IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void superhack::DonateStrategy::onTimer(const Timer *)
{
    setState(isActive() ? STATE_WAIT : STATE_CONNECT);
}


superhack::IClient *superhack::DonateStrategy::createProxy()
{
...
void superhack::DonateStrategy::idle(double min, double max)
{
    m_timer->start(random(m_idleTime, min, max), 0);
}


void superhack::DonateStrategy::setAlgorithms(rapidjson::Document &doc, rapidjson::Value &params)
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void superhack::DonateStrategy::setJob(IClient *client, const Job &job, const rapidjson::Value &params)
{
    if (isActive()) {
...
void superhack::DonateStrategy::setResult(IClient *client, const SubmitResult &result, const char *error)
{
    m_listener->onResultAccepted(this, client, result, error);
}


void superhack::DonateStrategy::setState(State state)
{
