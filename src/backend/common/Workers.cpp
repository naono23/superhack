...
{
public:
    superhack_DISABLE_COPY_MOVE(WorkersPrivate)
...
template<class T>
const superhack::Hashrate *superhack::Workers<T>::hashrate() const
{
...
template<class T>
void superhack::Workers<T>::setBackend(IBackend *backend)
{
...
template<class T>
void superhack::Workers<T>::start(const std::vector<T> &data)
{
...
template<class T>
void superhack::Workers<T>::tick(uint64_t)
{
    if (!d_ptr->hashrate) {
        return;
    }
...
template<class T>
superhack::IWorker *superhack::Workers<T>::create(Thread<T> *)
{
    return nullptr;
}
...
template<class T>
void superhack::Workers<T>::onReady(void *arg)
{
...
template<>
superhack::IWorker *superhack::Workers<CpuLaunchData>::create(Thread<CpuLaunchData> *handle)
{
...
#ifdef superhack_FEATURE_OPENCL
template<>
superhack::IWorker *superhack::Workers<OclLaunchData>::create(Thread<OclLaunchData> *handle)
{
...
#ifdef superhack_FEATURE_CUDA
template<>
superhack::IWorker *superhack::Workers<CudaLaunchData>::create(Thread<CudaLaunchData> *handle)
{
