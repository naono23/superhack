...
constexpr uint32_t superhack::CudaAstroBWTRunner::BWT_DATA_STRIDE;


superhack::CudaAstroBWTRunner::CudaAstroBWTRunner(size_t index, const CudaLaunchData &data) :
...
bool superhack::CudaAstroBWTRunner::run(uint32_t startNonce, uint32_t *rescount, uint32_t *resnonce)
{
    return callWrapper(CudaLib::astroBWTHash(m_ctx, startNonce, m_target, rescount, resnonce));
}


bool superhack::CudaAstroBWTRunner::set(const Job &job, uint8_t *blob)
{
...
size_t superhack::CudaAstroBWTRunner::roundSize() const
{
...
size_t superhack::CudaAstroBWTRunner::processedHashes() const
{
