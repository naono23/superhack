...
superhack::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
superhack::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool superhack::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value superhack::CudaThread::toJSON(rapidjson::Document &doc) const
{
