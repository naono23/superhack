...
bool superhack::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *superhack::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void superhack::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool superhack::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth superhack::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
