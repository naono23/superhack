...
superhack::CudaWorker::CudaWorker(size_t id, const CudaLaunchData &data) :
...
superhack::CudaWorker::~CudaWorker()
{
    delete m_runner;
}


void superhack::CudaWorker::jobEarlyNotification(const Job& job)
{
    if (m_runner) {
...
bool superhack::CudaWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t superhack::CudaWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void superhack::CudaWorker::start()
{
...
bool superhack::CudaWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::CUDA) == 0) {
        return false;
    }
...
void superhack::CudaWorker::storeStats()
{
    if (!isReady()) {
        return;
    }
