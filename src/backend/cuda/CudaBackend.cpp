...
#ifdef superhack_FEATURE_NVML
#include "backend/cuda/wrappers/NvmlLib.h"

namespace superhack { static const char *kNvmlLabel = "NVML"; }
#endif
...
const char *superhack::cuda_tag()
{
    return Tags::nvidia();
}


superhack::CudaBackend::CudaBackend(Controller *controller) :
...
superhack::CudaBackend::~CudaBackend()
{
    delete d_ptr;
...
bool superhack::CudaBackend::isEnabled() const
{
    return d_ptr->controller->config()->cuda().isEnabled() && CudaLib::isInitialized() && !d_ptr->devices.empty();;
}


bool superhack::CudaBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cuda().threads().get(algorithm).isEmpty();
}


const superhack::Hashrate *superhack::CudaBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const superhack::String &superhack::CudaBackend::profileName() const
{
    return d_ptr->profileName;
}


const superhack::String &superhack::CudaBackend::type() const
{
    return kType;
}


void superhack::CudaBackend::execCommand(char)
{
}


void superhack::CudaBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void superhack::CudaBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void superhack::CudaBackend::printHealth()
{
#   ifdef superhack_FEATURE_NVML
    d_ptr->printHealth();
#   endif
...
void superhack::CudaBackend::setJob(const Job &job)
{
...
void superhack::CudaBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void superhack::CudaBackend::stop()
{
...
void superhack::CudaBackend::tick(uint64_t ticks)
{
...
#ifdef superhack_FEATURE_API
rapidjson::Value superhack::CudaBackend::toJSON(rapidjson::Document &doc) const
{
...
void superhack::CudaBackend::handleRequest(IApiRequest &)
{
}
#endif
