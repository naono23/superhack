...
const char *superhack::backend_tag(uint32_t backend)
{
...
const char *superhack::cpu_tag()
{
    return Tags::cpu();
}


superhack::CpuBackend::CpuBackend(Controller *controller) :
...
superhack::CpuBackend::~CpuBackend()
{
    delete d_ptr;
}


bool superhack::CpuBackend::isEnabled() const
{
    return d_ptr->controller->config()->cpu().isEnabled();
}


bool superhack::CpuBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cpu().threads().get(algorithm).isEmpty();
}


const superhack::Hashrate *superhack::CpuBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const superhack::String &superhack::CpuBackend::profileName() const
{
    return d_ptr->profileName;
}


const superhack::String &superhack::CpuBackend::type() const
{
    return kType;
}


void superhack::CpuBackend::prepare(const Job &nextJob)
{
#   ifdef superhack_ALGO_ARGON2
    const superhack::Algorithm::Family f = nextJob.algorithm().family();
...
void superhack::CpuBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void superhack::CpuBackend::printHealth()
{
}


void superhack::CpuBackend::setJob(const Job &job)
{
    if (!isEnabled()) {
        return stop();
    }
...
void superhack::CpuBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void superhack::CpuBackend::stop()
{
...
void superhack::CpuBackend::tick(uint64_t ticks)
{
...
#ifdef superhack_FEATURE_API
rapidjson::Value superhack::CpuBackend::toJSON(rapidjson::Document &doc) const
{
...
void superhack::CpuBackend::handleRequest(IApiRequest &request)
{
