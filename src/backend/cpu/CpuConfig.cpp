...
bool superhack::CpuConfig::isHwAES() const
{
    return (m_aes == AES_AUTO ? (Cpu::info()->hasAES() ? AES_HW : AES_SOFT) : m_aes) == AES_HW;
}


rapidjson::Value superhack::CpuConfig::toJSON(rapidjson::Document &doc) const
{
...
size_t superhack::CpuConfig::memPoolSize() const
{
    return m_memoryPool < 0 ? Cpu::info()->threads() : m_memoryPool;
}


std::vector<superhack::CpuLaunchData> superhack::CpuConfig::get(const Miner *miner, const Algorithm &algorithm) const
{
...
void superhack::CpuConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void superhack::CpuConfig::generate()
{
    ...
    count += superhack::generate<Algorithm::CN>(m_threads, m_limit);
    count += superhack::generate<Algorithm::CN_LITE>(m_threads, m_limit);
    count += superhack::generate<Algorithm::CN_HEAVY>(m_threads, m_limit);
    count += superhack::generate<Algorithm::CN_PICO>(m_threads, m_limit);
    count += superhack::generate<Algorithm::RANDOM_X>(m_threads, m_limit);
    count += superhack::generate<Algorithm::ARGON2>(m_threads, m_limit);
    count += superhack::generate<Algorithm::ASTROBWT>(m_threads, m_limit);

    m_shouldSave |= count > 0;
}


void superhack::CpuConfig::setAesMode(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
void superhack::CpuConfig::setMemoryPool(const rapidjson::Value &value)
{
    if (value.IsBool()) {
