...
superhack::CpuLaunchData::CpuLaunchData(const Miner *miner, const Algorithm &algorithm, const CpuConfig &config, const CpuThread &thread) :
    algorithm(algorithm),
...
bool superhack::CpuLaunchData::isEqual(const CpuLaunchData &other) const
{
...
superhack::CnHash::AlgoVariant superhack::CpuLaunchData::av() const
{
    if (intensity <= 2) {
...
const char *superhack::CpuLaunchData::tag()
{
    return cpu_tag();
}
