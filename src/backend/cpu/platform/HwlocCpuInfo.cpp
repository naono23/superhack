...
superhack::HwlocCpuInfo::HwlocCpuInfo()
{
    m_threads = 0;
...
bool superhack::HwlocCpuInfo::membind(hwloc_const_bitmap_t nodeset)
{
    if (!hwloc_topology_get_support(m_topology)->membind->set_thisthread_membind) {
        return false;
    }
...
superhack::CpuThreads superhack::HwlocCpuInfo::threads(const Algorithm &algorithm, uint32_t limit) const
{
...
superhack::CpuThreads superhack::HwlocCpuInfo::allThreads(const Algorithm &algorithm, uint32_t limit) const
{
    CpuThreads threads;
    threads.reserve(m_threads);
...
void superhack::HwlocCpuInfo::processTopLevelCache(hwloc_obj_t cache, const Algorithm &algorithm, CpuThreads &threads, size_t limit) const
{
#   ifndef superhack_ARM
