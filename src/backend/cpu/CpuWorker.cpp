...
template<size_t N>
superhack::CpuWorker<N>::CpuWorker(size_t id, const CpuLaunchData &data) :
...
template<size_t N>
superhack::CpuWorker<N>::~CpuWorker()
{
#   ifdef superhack_ALGO_RANDOMX
    RxVm::destroy(m_vm);
#   endif
...
#ifdef superhack_ALGO_RANDOMX
template<size_t N>
void superhack::CpuWorker<N>::allocateRandomX_VM()
{
...
template<size_t N>
bool superhack::CpuWorker<N>::selfTest()
{
#   ifdef superhack_ALGO_RANDOMX
...
template<size_t N>
void superhack::CpuWorker<N>::start()
{
...
template<size_t N>
bool superhack::CpuWorker<N>::verify(const Algorithm &algorithm, const uint8_t *referenceValue)
{
...
template<size_t N>
bool superhack::CpuWorker<N>::verify2(const Algorithm &algorithm, const uint8_t *referenceValue)
{
    cn_hash_fun func = fn(algorithm);
    if (!func) {
        return false;
...
template<size_t N>
void superhack::CpuWorker<N>::allocateCnCtx()
{
    if (m_ctx[0] == nullptr) {
...
template<size_t N>
void superhack::CpuWorker<N>::consumeJob()
{
    if (Nonce::sequence(Nonce::CPU) == 0) {
        return;
    }
