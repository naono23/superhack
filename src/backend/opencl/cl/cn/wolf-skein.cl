...
#   define superhack_amd_bitalign(src0, src1, src2) amd_bitalign(src0, src1, src2)
#else
 ...
 */
inline uint2 superhack_amd_bitalign(const uint2 src0, const uint2 src1, const uint src2)
{
    uint2 result;
...
{
    if (y < 32) {
        return(as_ulong(superhack_amd_bitalign(x, x.s10, 32 - y)));
    }
    else {
        return(as_ulong(superhack_amd_bitalign(x.s10, x, 32 - (y - 32))));
    }
}
