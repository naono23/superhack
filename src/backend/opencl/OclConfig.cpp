...
#ifndef superhack_OS_APPLE
superhack::OclConfig::OclConfig() : m_platformVendor(kAMD) {}
#else
superhack::OclConfig::OclConfig() = default;
#endif


superhack::OclPlatform superhack::OclConfig::platform() const
{
...
rapidjson::Value superhack::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<superhack::OclLaunchData> superhack::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices) const
{
...
void superhack::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void superhack::OclConfig::generate()
{
    ...
    count += superhack::generate<Algorithm::CN>(m_threads, devices);
    count += superhack::generate<Algorithm::CN_LITE>(m_threads, devices);
    count += superhack::generate<Algorithm::CN_HEAVY>(m_threads, devices);
    count += superhack::generate<Algorithm::CN_PICO>(m_threads, devices);
    count += superhack::generate<Algorithm::RANDOM_X>(m_threads, devices);
    count += superhack::generate<Algorithm::ASTROBWT>(m_threads, devices);
    count += superhack::generate<Algorithm::KAWPOW>(m_threads, devices);

    m_shouldSave = count > 0;
}


void superhack::OclConfig::setDevicesHint(const char *devicesHint)
{
...
#ifndef superhack_OS_APPLE
void superhack::OclConfig::setPlatform(const rapidjson::Value &platform)
{
