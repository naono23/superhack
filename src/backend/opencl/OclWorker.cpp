...
superhack::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
superhack::OclWorker::~OclWorker()
{
    delete m_runner;
}


void superhack::OclWorker::jobEarlyNotification(const Job& job)
{
    if (m_runner) {
...
bool superhack::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t superhack::OclWorker::intensity() const
{
    return m_runner ? m_runner->intensity() : 0;
}


void superhack::OclWorker::start()
{
...
bool superhack::OclWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void superhack::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
