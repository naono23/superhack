...
cl_program superhack::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\superhack\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string superhack::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string superhack::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void superhack::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
