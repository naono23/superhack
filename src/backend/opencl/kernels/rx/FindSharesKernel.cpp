...
void superhack::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void superhack::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void superhack::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void superhack::FindSharesKernel::setNonce(uint32_t nonce)
{
