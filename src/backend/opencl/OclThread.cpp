...
superhack::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool superhack::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value superhack::OclThread::toJSON(rapidjson::Document &doc) const
{
