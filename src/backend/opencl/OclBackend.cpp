...
namespace superhack { static const char *kAdlLabel = "ADL"; }
#endif
...
const char *superhack::ocl_tag()
{
    return Tags::opencl();
}


superhack::OclBackend::OclBackend(Controller *controller) :
...
superhack::OclBackend::~OclBackend()
{
    delete d_ptr;
...
bool superhack::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool superhack::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const superhack::Hashrate *superhack::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const superhack::String &superhack::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const superhack::String &superhack::OclBackend::type() const
{
    return kType;
}


void superhack::OclBackend::execCommand(char)
{
}


void superhack::OclBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void superhack::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void superhack::OclBackend::printHealth()
{
#   ifdef superhack_FEATURE_ADL
    d_ptr->printHealth();
#   endif
...
void superhack::OclBackend::setJob(const Job &job)
{
...
void superhack::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void superhack::OclBackend::stop()
{
...
void superhack::OclBackend::tick(uint64_t ticks)
{
...
#ifdef superhack_FEATURE_API
rapidjson::Value superhack::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void superhack::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
