...
private:
    ...
    std::string getSource(uint64_t offset) const
    {
        ...
        for (size_t i = 0; i < OclCnR::kHeightChunkSize; ++i) {
            ...
            const std::string kernel = std::regex_replace(std::string(cryptonight_r_cl), std::regex("superhack_INCLUDE_RANDOM_MATH"), getCode(code, code_size));
...
cl_program superhack::OclCnR::get(const IOclRunner &runner, uint64_t height)
{
