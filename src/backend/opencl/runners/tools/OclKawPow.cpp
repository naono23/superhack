...
private:
    ...
    std::string getSource(uint64_t prog_seed) const
    {
        ...
        std::string kernel = std::regex_replace(std::string(kawpow_cl), std::regex("superhack_INCLUDE_PROGPOW_RANDOM_MATH"), ret.str());
        ret.str(std::string());
        ...
        kernel = std::regex_replace(kernel, std::regex("superhack_INCLUDE_PROGPOW_DATA_LOADS"), ret.str());
        return kernel;
    }
