...
superhack::OclSharedData &superhack::OclSharedState::get(uint32_t index)
{
    return map[index];
}


void superhack::OclSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void superhack::OclSharedState::start(const std::vector<OclLaunchData> &threads, const Job &job)
{
    assert(map.empty());
