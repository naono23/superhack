...
cl_mem superhack::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset, size_t limit)
{
...
uint64_t superhack::OclSharedData::adjustDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
#   ifdef superhack_INTERLEAVE_DEBUG
...
uint64_t superhack::OclSharedData::resumeDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
void superhack::OclSharedData::release()
{
...
void superhack::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void superhack::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef superhack_ALGO_RANDOMX
cl_mem superhack::OclSharedData::dataset() const
{
    if (!m_dataset) {
...
void superhack::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
