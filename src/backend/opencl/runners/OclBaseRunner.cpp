...
superhack::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
#   ifdef superhack_STRICT_OPENCL_CACHE
    m_deviceKey += ":";
...
superhack::OclBaseRunner::~OclBaseRunner()
{
...
size_t superhack::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t superhack::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void superhack::OclBaseRunner::build()
{
...
void superhack::OclBaseRunner::init()
{
...
cl_mem superhack::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t superhack::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void superhack::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void superhack::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void superhack::OclBaseRunner::finalize(uint32_t *hashOutput)
{
