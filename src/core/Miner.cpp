...
{
public:
    superhack_DISABLE_COPY_MOVE_DEFAULT(MinerPrivate)
...
superhack::Miner::Miner(Controller *controller)
...
superhack::Miner::~Miner()
{
    delete d_ptr;
}


bool superhack::Miner::isEnabled() const
{
    return d_ptr->enabled;
}


bool superhack::Miner::isEnabled(const Algorithm &algorithm) const
{
    return std::find(d_ptr->algorithms.begin(), d_ptr->algorithms.end(), algorithm) != d_ptr->algorithms.end();
}


const superhack::Algorithms &superhack::Miner::algorithms() const
{
    return d_ptr->algorithms;
}


const std::vector<superhack::IBackend *> &superhack::Miner::backends() const
{
    return d_ptr->backends;
}


superhack::Job superhack::Miner::job() const
{
...
void superhack::Miner::execCommand(char command)
{
    switch (command) {
    case 'h':
    case 'H':
...
void superhack::Miner::setEnabled(bool enabled)
{
...
void superhack::Miner::setJob(const Job &job, bool donate)
{
...
void superhack::Miner::onConfigChanged(Config *config, Config *previousConfig)
{
    d_ptr->rebuild();
...
void superhack::Miner::onTimer(const Timer *)
{
...
#ifdef superhack_FEATURE_API
void superhack::Miner::onRequest(IApiRequest &request)
{
...
#ifdef superhack_ALGO_RANDOMX
void superhack::Miner::onDatasetReady()
{
    if (!Rx::isReady(job())) {
        return;
    }
